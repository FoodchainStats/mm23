...
tables' spreadsheet. So there is now a new function `get_cpih_weights` which
generates a weights dataset. It may not be very robust, it needs some testing,
but if everything is in the right place it works OK.
...
Note that 'all items' doesn't have to be all items CPIH, it can be a section
(such as food).
