...
#'
#' Contains the url endings for all the price quote downloads for 2010-2019.
...
#' }
get_cpi_price_quotes_archive <- function(year, foodonly = TRUE) {

  if(max(year) >=2020 | min(year) <= 2009) {stop("Year must be in the range 2010-2019")}
  ...
  stubs <- archive_urls() |> dplyr::filter(yr %in% year) |> dplyr::select(stub) |> as.list()
...
#'
#' This is older, more 'hardcoded' and was crashing on some computers with
#' limited memory. The elegantly named get_cpi_price_quotes_archive() will do
#' the same thing but for specific years.
#'
